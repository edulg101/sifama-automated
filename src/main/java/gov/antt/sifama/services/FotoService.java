...
public class FotoService {
    ...
    public void unzipAllDirectory(String path, String destPath) throws IOException {
        String[] files = null;
        String[] desFiles = null;
        try {
            files = getFilesInPath(path);
            desFiles = getFilesInPath(destPath);
        ...
        // delete all files in destDirectory
        for (int i = 0; i < desFiles.length; i++) {
            File file = new File(destPath + File.separator + desFiles[i]);
            if (file.exists()){
                if(file.delete()){
                    System.out.println("arquivo antigo apagado");
                };
            }
        }
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            //chance to unzip for unzip in subfolders
    ...
    public void insertCaption() throws IOException {
        ...
        for(Foto foto: allFotos){
            if(foto.getLocal() != null){
                ...
                createImage(img, createRect(foto.getLocal().getObservacao()), filePath);
            }
        }
    ...
    public BufferedImage createRect(String captionText){
        BufferedImage newImage = new BufferedImage(500, 40, BufferedImage.TYPE_INT_RGB);
        Graphics2D g2 = newImage.createGraphics();
        Color oldColor = g2.getColor();
        g2.setPaint(Color.WHITE);
